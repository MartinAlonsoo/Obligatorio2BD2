...
-- Tabla para registrar cada vez que un personaje sube de nivel
CREATE TABLE Log_Aumento_Nivel (
    id_log          NUMBER GENERATED BY DEFAULT AS IDENTITY,
    emailJugador    VARCHAR2(40) NOT NULL,
    nivel_anterior  NUMBER(10) NOT NULL,
    nivel_nuevo     NUMBER(10) NOT NULL,
    fecha_aumento   DATE DEFAULT SYSDATE NOT NULL,
    CONSTRAINT pk_Log_Aumento_Nivel PRIMARY KEY (id_log),
    CONSTRAINT fk_Log_Aumento_Nivel_Pers FOREIGN KEY (emailJugador) REFERENCES Personaje(email_Jugador)
);

-- Tabla para evitar que el premio se entregue dos veces en el mismo día
CREATE TABLE Log_Premio_Diario (
    id_log_premio   NUMBER GENERATED BY DEFAULT AS IDENTITY,
    emailJugador    VARCHAR2(40) NOT NULL,
    fecha_premio    DATE DEFAULT SYSDATE NOT NULL,
    item_recibido   VARCHAR2(20) NOT NULL,
    CONSTRAINT pk_Log_Premio_Diario PRIMARY KEY (id_log_premio),
    CONSTRAINT fk_Log_Premio_Diario_Pers FOREIGN KEY (emailJugador) REFERENCES Personaje(email_Jugador),
    CONSTRAINT fk_Log_Premio_Diario_Item FOREIGN KEY (item_recibido) REFERENCES Items(nombre)
);
